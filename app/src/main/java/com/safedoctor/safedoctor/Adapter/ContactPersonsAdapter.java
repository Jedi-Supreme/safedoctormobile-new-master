...
public class ContactPersonsAdapter  extends RecyclerView.Adapter<ContactPersonsAdapter.ViewHolder> {
    ...
    public ContactPersonsAdapter(Context mContext, List<Patientcontactperson> items) {
        this.ctx = mContext;
        this.items = items;
    ...
    public void onBindViewHolder(final ViewHolder holder, final int position) {
        final Patientcontactperson contactperson = items.get(position);

        // displaying text view data

        holder.from.setText(contactperson.getName());
        holder.email.setText(contactperson.getEmails());
        holder.phonenumber.setText(contactperson.getContactnumbers());

        BasicObject basicobj = AuxUtils.getBaseObjectByValue(AppConstants.CACHE_FORMDATA.getRelation(),
                contactperson.getRelationshipid());

        holder.relationship.setText(basicobj.getName());
        if(contactperson.getName() != null && contactperson.getName().length() > 1) {
            holder.image_letter.setText(contactperson.getName().substring(0, 1));
        }
        ...
        holder.lyt_parent.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                onClickListener.onItemClick(v, contactperson, position);
            }
        });
        ...
        holder.lyt_parent.setOnLongClickListener(new View.OnLongClickListener() {
            ...
            public boolean onLongClick(View v) {
                ...
                onClickListener.onItemLongClick(v, contactperson, position);
                return true;
            }
        });

        toggleCheckedIcon(holder, position);
        displayImage(holder, contactperson);

    }

    private void displayImage(ViewHolder holder, Patientcontactperson contactperson) {
        if (contactperson.image != null) {
            Tools.displayImageRound(ctx, holder.image, contactperson.image);
            holder.image.setColorFilter(null);
            holder.image_letter.setVisibility(View.GONE);
        } else {
            holder.image.setImageResource(R.drawable.shape_circle);
            holder.image.setColorFilter(contactperson.color);
    ...
    public Patientcontactperson getItem(int position) {
    ...
    {
        void onItemClick(View view, Patientcontactperson obj, int pos);

        void onItemLongClick(View view, Patientcontactperson obj, int pos);
    }
